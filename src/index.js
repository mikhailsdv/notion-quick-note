	...
	ctx.replyWithMarkdown(
		trimMessage(`
			...
			*Возможности бота:*
			...
			Шорткат чата с ботом можно положить на главный экран и не ждать 7-8 секунд, пока запускается Notion.
...
bot.on("message", async ctx => {
	...
	if (!text && !photoUrl) {
		return ctx.reply("❌ В этом сообщении нет текста или фото")
	}
	...
	try {
		...
		console.log(err)
		await telegram.editMessageText(chatId, messageFromBot.message_id, null, "❌ Во время сохранения заметки произошла ошибка. Попробуйте прислать тот же текст еще раз.")
	}
})
